...
%                Frs(end+(1-nsh:0),1:nwin-1) = diag(sparse(taper))*Frs(end+(1-nsh:0),1:nwin-1);
        ...
        function [data,fs] = signal(me,columnfilter,hilbert)
            ...
            % Taper is defined so that h(k) + h(k+bw) = 1
            sh = F(nnyq-nsh+1:nnyq,1:end-1);
            F(1:nsh,2:end) = F(1:nsh,2:end)+sh;
            F(nnyq-nsh+1:end,:) = [];
            Ffull(noffset+(1:numel(F))) = F(:);
            Ffull(me.fullN) = 0;
